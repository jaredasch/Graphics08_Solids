...
void scanline_convert( struct matrix *points, int i, screen s, zbuffer zb ) {
    ...
    if( points->m[1][i] >= points->m[1][i+1] && points->m[1][i] >= points->m[1][i+2] ){
        ...
        if( points->m[1][i+1] > points->m[1][i+2] ){
            mx = points->m[0][i+1]; my = points->m[1][i+1]; mz = points->m[2][i+1];
            bx = points->m[0][i+2]; by = points->m[1][i+2]; bz = points->m[2][i+2];
        } else {
            mx = points->m[0][i+2]; my = points->m[1][i+2]; mz = points->m[2][i+2];
            bx = points->m[0][i+1]; by = points->m[1][i+1]; bz = points->m[2][i+1];
        }
    } else if( points->m[1][i+1] >= points->m[1][i] && points->m[1][i+1] >= points->m[1][i+2] ) {
        tx = points->m[0][i+1]; ty = points->m[1][i+1]; tz = points->m[2][i+1];
        if( points->m[1][i] > points->m[1][i+2] ){
            mx = points->m[0][i]; my = points->m[1][i]; mz = points->m[2][i];
            bx = points->m[0][i+2]; by = points->m[1][i+2]; bz = points->m[2][i+2];
        } else {
            mx = points->m[0][i+2]; my = points->m[1][i+2]; mz = points->m[2][i+2];
            bx = points->m[0][i]; by = points->m[1][i]; bz = points->m[2][i];
        }
    } else {
        tx = points->m[0][i+2]; ty = points->m[1][i+2]; tz = points->m[2][i+2];
